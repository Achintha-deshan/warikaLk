...
export default function SignupPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setLoading(true)
    try { await api.post('/auth/signup', { ...form, phone }); navigate('/daily-report', { replace: true }) }
  ...
  return <AuthLayout eyebrow={t('auth.start')} title={t('auth.signupTitle')} intro={t('auth.signupIntro')}>
    <form className="auth-form" onSubmit={sent ? submit : sendCode} noValidate>
      <Field label={t('auth.mobile')} error={getFieldError(errors, 'phone')}>
        <div className="inline-field"><input value={phone} onChange={(event) => setPhone(event.target.value)} readOnly={sent} inputMode="tel" autoComplete="tel" placeholder="07X XXX XXXX" /><button type="button" className="text-button" onClick={changeNumber} hidden={!sent}>{t('auth.change')}</button></div>
      </Field>
      {sent && <>
        <div className="form-grid"><Field label={t('auth.business')} error={getFieldError(errors, 'businessName')}><input value={form.businessName} onChange={(event) => update('businessName', event.target.value)} autoComplete="organization" /></Field><Field label={t('auth.owner')} error={getFieldError(errors, 'ownerName')}><input value={form.ownerName} onChange={(event) => update('ownerName', event.target.value)} autoComplete="name" /></Field></div>
        <div className="form-grid"><Field label={t('auth.password')} error={getFieldError(errors, 'password')}><input type="password" value={form.password} onChange={(event) => update('password', event.target.value)} autoComplete="new-password" /></Field><Field label={t('auth.confirmPassword')} error={getFieldError(errors, 'confirmPassword')}><input type="password" value={form.confirmPassword} onChange={(event) => update('confirmPassword', event.target.value)} autoComplete="new-password" /></Field></div>
        ...
        <Field label={t('auth.verification')} error={getFieldError(errors, 'otp')}><input value={form.otp} onChange={(event) => update('otp', event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" maxLength={6} /><button type="button" className="text-button" disabled={cooldown > 0 || loading} onClick={() => void sendCode({ preventDefault: () => undefined } as FormEvent)}>{cooldown > 0 ? `Resend in ${cooldown}s` : t('auth.sendCode')}</button></Field>
      </>}
      <ApiNotice message={notice} />
      <button className="primary-button" disabled={loading} type="submit">{loading ? <Spinner /> : sent ? t('auth.createWorkspace') : t('auth.sendCode')}<span aria-hidden="true">↗</span></button>
    </form>
    <p className="auth-switch">{t('auth.already')} <Link to="/login">{t('auth.login')}</Link></p>
  </AuthLayout>
}